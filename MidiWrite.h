...
void set_midioptions(int tpq, int track, int channel, int instrument, MidiFile *midifile);
//instrument 0-127
void creat_midifile(vector<vector<int>> *tongueData, vector<vector<int>> *speedData, MidiFile *midifile, int channel, int track, int duration, int tpq);
//duration 1-8
void write_midifile(string *fileName, MidiFile *midiFile);
...
void set_midioptions(int tpq, int track, int channel, int instrument, MidiFile *midifile)
{
    ...
    for(int i=0; i<track; i++)
    {
      ...
      {
	(*midifile).addTimbre(i, 0, j, instrument);
      }
    }
}
...
void creat_midifile(vector<vector<int>> *tongueData, vector<vector<int>> *speedData, MidiFile *midifile, int channel, int track, int duration, int tpq)
{
    int noteNum, startTime, startTick, endTick, key, velocity;
    noteNum = (*tongueData).size();
    for(int i=0; i<track; i++)
    {
	for(int j=0; j<channel; j++)
	{
	    ...
	    {
		startTick = int(startTime / 4.0 * tpq);
		key = (*tongueData)[k][j];
		velocity = (k==noteNum-1) ? 0 : (*speedData)[k][j];
		endTick = startTick + int(duration / 4.0 * tpq);
		startTick = endTick;
		(*midifile).addNoteOn (i, startTick, j, key, velocity);
		(*midifile).addNoteOff(i, endTick, j, key);
	    }
	}
    }
}
...
void write_midifile(string fileName, MidiFile *midiFile)
{
    ...
    if (finalName.empty()) cout << *midiFile;
      //midifile.writeHex(cout);
    else (*midiFile).write(finalName);
}
